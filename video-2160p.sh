...
static_params="-c:a libfdk_aac -ar 48000 -c:v h264_nvenc -preset slow -multipass fullres -extra_sei 0 -b:v 0 -cq 1 -sc_threshold 0"
static_params+=" -g ${key_frames_interval} -keyint_min ${key_frames_interval} -hls_time ${segment_target_duration}"
static_params+=" -hls_playlist_type vod"

# misc params
misc_params="-hide_banner -sn -an -probesize 250M -y"
...
for rendition in "${renditions[@]}"; do
  ...
  cmd+=" ${static_params} -ac ${channel} -vf hwupload_cuda,scale_cuda=${width}:${height}:force_original_aspect_ratio=decrease"
  cmd+=" -b:v ${bitrate} -maxrate ${maxrate%.*}k -bufsize ${bufsize%.*}k -b:a ${audiorate}"
  cmd+=" -hls_segment_filename ${target}/${name}_%03d.ts ${target}/${name}.m3u8"

  # add rendition entry in the master playlist
  master_playlist+='#EXT-X-STREAM-INF:BANDWIDTH='${bandwidth}',RESOLUTION='${resolution}',AUDIO='\"${name}\"'\n'${name}'.m3u8\n'
done

#encoding
ffmpeg -an ${misc_params} -i ${source} ${cmd}

# create master playlist file for videos
echo -e "${master_playlist}" > ${target}/playlist.m3u8

exit

#Not runnign this part because we still need audio!
