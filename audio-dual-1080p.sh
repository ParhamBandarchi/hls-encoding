...
languages=($(mediainfo --Inform="Audio;%Language/String4%\n" ${source} | sed '/^$/d')) # Getting the list of languages
...
for rendition in "${renditions[@]}"; do
  ...
  for lang in "${languages[@]}"; do
    cmd+=" ${static_params} -map 0:a:$index -ac ${channel}"
    cmd+=" -b:a ${audiorate}"
    cmd+=" -hls_segment_filename ${target}/${name}_${lang}_%03d.ts ${target}/${name}_${lang}.m3u8"

    # add rendition entry in the master playlist
    master_playlist+='#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID='\"${height}p\"',LANGUAGE='\"${lang}\"',NAME="audio",CHANNELS='\"${channel}\"',URI='\"${name}_${lang}.m3u8\"'\n'
